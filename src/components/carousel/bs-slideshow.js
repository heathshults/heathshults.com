/* eslint no-unused-vars: "off", wrap-iife: "off", vars-on-top: "off", no-empty: "off", one-var: "off", no-multi-assign: "off", complexity: "off" */
...
(function ($) {
    ...
    var setFieldIndex = function (items) {
        ...
        for (i = 0; i < items[0].length; i++) {
            totalItems += items[0][i].itemDivWidth || 1
        }
    ...
    bsCarousel.item_DEFAULTS = {
        ...
        title: undefined,
        image: undefined,
        imageX: undefined, // left: integers | right: integers
        imageY: undefined, // top, integers | bottom, integers
        titleTooltip: undefined,
        'class': undefined,
        align: undefined, // left, right, center
        alignSelf: undefined, // flex-start, center, stretch, flex-end
        alignItems: undefined, // flex-start, center, stretch, flex-end
        width: undefined,
        sortCarousel: false,
        order: 'asc', // asc, desc
        visible: true,
        formatter: undefined,
        events: undefined,
        link: undefined, // http://hello.com
        linkTarget: undefined, // _blank, _self
    }
    ...
    bsCarousel.prototype.initContainer = function () {
        this.$container = $([
            ...
            '</div>',
            '<ol class="carousel-indicators"><li></li></ol>',
        ...
        this.$crslContainer = this.$container.find('.carousel .slide')
        this.$crslIndicators = this.$container.find('.carousel-indicators')
        this.$crslInner = this.$container.find('.carousel-inner')
        this.$crslItem = this.$container.find('.item')
        this.$crslLoading = this.$container.find('.carousel-loading')
        this.$crslTitle = this.$container.find('.carousel-caption')
        this.$crslNavLeft = this.$container.find('.left .carousel-control')
        this.$crslNavRight = this.$container.find('.right .carousel-control')
        this.$pagination = this.$container.find('ol.carousel-indicators li')

        this.$crslInner.append(this.$el)
    ...
    bsCarousel.prototype.initCarousel = function () {
        ...
        this.$crslInner = this.$el.find('.carousel-inner')
        if (!this.$crslInner.length) {
            this.$crslInner = $('<span class="delete-me-heathen" hidden></span>').appendTo(this.$el)
        }
        this.$crslInner.find('div.item').each(function () {
            var item = []
            ...
            $(this).find('.item').each(function () {
                ...
                this.$crslIndicators.append('<li data-target="#carousel-example-generic" data-slide-to="0" class="indi-circ"></li>')
            })
            items.push(item)
        })
        ...
        this.$el.find('>tbody>tr').each(function (y) {
            ...
            $(this).find('>td').each(function (x) {
                var $this = $(this),
                    cspan = +$this.attr('itemDivWidth') || 1,
                ...
                row['_' + field + '_itemDivWidth'] = $(this).attr('itemDivWidth')
    ...
    bsCarousel.prototype.initcrslInner = function () {
        var that = this,
            visibleitems = {},
            html = []
        ...
        $.each(this.options.items, (i, items) => {
            ...
            $.each(items, function (j, item) {
                ...
                if (typeof item.fieldIndex !== 'undefined') {
                    that.crslInner.fields[item.fieldIndex] = item.field
                    that.crslInner.styles[item.fieldIndex] = align + style
                    that.crslInner.classes[item.fieldIndex] = class_
                    that.crslInner.formatters[item.fieldIndex] = item.formatter
                    that.crslInner.events[item.fieldIndex] = item.events
                    that.crslInner.sorters[item.fieldIndex] = item.sorter
                    that.crslInner.sortNames[item.fieldIndex] = item.sortName
                    that.crslInner.cellStyles[item.fieldIndex] = item.cellStyle
                    that.crslInner.searchables[item.fieldIndex] = item.searchable
                ...
                html.push('<div class="carousel-caption"' + sprintf(' title="%s"', item.titleTooltip),
                    item.field ?
                        sprintf(' class="bs-checkbox %s"', item['class'] || '') :
                        class_,
                    sprintf(' style="%s"', halign + style),
                    sprintf(' rowspan="%s"', item.rowspan),
                    sprintf(' itemDivWidth="%s"', item.itemDivWidth),
                ...
                // if (item.checkbox) {
                //   if (!that.options.singleSelect && that.options.checkboxcrslInner) {
                //     text = '<input name="btSelectAll" type="checkbox" />'
                //   }
                //   that.crslInner.stateField = item.field
                // }
                // if (item.radio) {
                //   text = ''
                //   that.crslInner.stateField = item.field
        ...
        this.$crslInner.html(html.join(''))
        this.$crslInner.find('div[data-field]').each(function (i) {
        ...
        this.$crslInner.children().children().off('keypress').on('keypress', function (event) {
        ...
        if (!this.options.showcrslInner) {
            ...
            this.$carouselLoading.css('top', this.$crslInner.outerHeight() + 1)
            // Assign the correct sortCarousel arrow
            // this.getCaret()
            $(window).on('resize.carousel', $.proxy(this.resetWidth, this))
        }

        this.$selectAll = this.$crslInner.find('[name="btSelectAll"]')
    ...
    bsCarousel.prototype.initFooter = function () {
        if (!this.options.showFooter || this.options.cardView) {
            this.$crslFooter.hide()
        } else {
            this.$crslFooter.show()
        }
    }
   ...
   */
    ...
    bsCarousel.prototype.initSort = function () {
        var that = this,
            ...
            index = $.inArray(this.options.sortName, this.crslInner.fields),
            timeoutId = 0
        ...
        if (index !== -1) {
            ...
            this.data.sort((a, b) => {
                if (that.crslInner.sortNames[index]) {
                    name = that.crslInner.sortNames[index]
                }
                var aa = getItemField(a, name, that.options.escape),
                    bb = getItemField(b, name, that.options.escape),
                    value = calculateObjectValue(that.crslInner, that.crslInner.sorters[index], [aa, bb])
            ...
            if (this.options.sortClass !== undefined) {
                ...
                timeoutId = setTimeout(() => {
                    ...
                    var index = that.$crslInner.find(sprintf('[data-field="%s"]',
    ...
    bsCarousel.prototype.onSort = function (event) {
        var $this = event.type === 'keypress' ? $(event.currentTarget) : $(event.currentTarget).parent(),
            $this_ = this.$crslInner.find('th').eq($this.index())

        this.$crslInner.add(this.$crslInner_).find('span.order').remove()
    ...
    var heathenHideToolbarComments = function () {
    ...
    //       for (var j = 0; j < that.crslInner.fields.length; j++) {

    //         if (!that.crslInner.searchables[j]) {
    //           continue
    //         }

    //         var key = $.isNumeric(that.crslInner.fields[j]) ? parseInt(that.crslInner.fields[j], 10) : that.crslInner.fields[j]
    ...
    //               that.crslInner.formatters[j], [value, item, i], value)
    //           }
    //         } else {
    //           value = item[key]
    //         }
    ...
    bsCarousel.prototype.initRow = function (item, i, data, parentDom) {
        ...
        if (this.options.cardView) {
            html.push(sprintf('<div itemDivWidth="%s"><div class="card-views">', this.crslInner.fields.length))
        }
        ...
        $.each(this.crslInner.fields, (j, field) => {
            var text = '',
                ...
                cellStyle = {},
                id_ = '',
                class_ = that.crslInner.classes[j],
                data_ = '',
                rowspan_ = '',
                itemDivWidth_ = '',
                title_ = '',
            ...
            style = sprintf('style="%s"', csses.concat(that.crslInner.styles[j]).join('; '))
            ...
            if (item['_' + field + '_itemDivWidth']) {
                itemDivWidth_ = sprintf(' itemDivWidth="%s"', item['_' + field + '_itemDivWidth'])
            }
            if (item['_' + field + '_title']) {
                title_ = sprintf(' title="%s"', item['_' + field + '_title'])
            }
            cellStyle = calculateObjectValue(that.crslInner,
                that.crslInner.cellStyles[j], [value_, item, i, field], cellStyle)
            ...
            if (cellStyle.css) {
                ...
                style = sprintf('style="%s"', csses_.concat(that.crslInner.styles[j]).join('; '))
            }

            value = calculateObjectValue(item,
                that.crslInner.formatters[j], [value_, item, i, field], value_)
            ...
            if (item.checkbox || item.radio) {
                ...
                text = [sprintf(that.options.cardView ?
                    '<div class="card-view %s">' : '<div class="bs-checkbox %s">', item['class'] || ''),
                '<input' +
          sprintf(' data-index="%s"', i) +
          ...
          ' />',
                that.crslInner.formatters[j] && typeof value === 'string' ? value : '',
                that.options.cardView ? '</div>' : '</div>'
                ].join('')

                item[that.crslInner.stateField] = value === true || (value && value.checked)
            } else {
                value = typeof value === 'undefined' || value === null ?
                    that.options.undefinedText : value

                text = that.options.cardView ? ['<div class="card-view">',
                    that.options.showcrslInner ? sprintf('<span class="title" %s>%s</span>', style,
                        getPropertyFromOther(that.items, 'field', 'title', field)) : '',
                    sprintf('<span class="value">%s</span>', value),
                    '</div>'
                ].join('') : [sprintf('<td%s %s %s %s %s %s %s>',
                    id_, class_, style, data_, rowspan_, itemDivWidth_, title_),
                value,
                '</div>'
                ].join('')
    ...
    bsCarousel.prototype.initBody = function (fixedScroll) {
        ...
        sprintf('<div itemDivWidth="%s"><div class="fixed-carousel-loading"><i class="fa fa-spinner fa-pulse fa-fw mr-2"></i>' + this.options.formatLoadingMessage() + '</div>  %s</div>',
            this.$crslInner.find('div').length,
        ...
        this.$body.find('> div[data-index] > div > .detail-icon').off('click').on('click', function () {
            ...
            if ($divtr.next().is('div.detail-view')) {
                ...
                $divtr.after(sprintf('<div class="detail-view"><div itemDivWidth="%s"></div></div>', $divtr.find('div').length))
        ...
        this.$selectItem.off('click').on('click', function (event) {
            ...
            if (that.options.maintainSelected && $(this).is(':radio')) {
                $.each(that.options.data, (i, row) => {
                    row[that.crslInner.stateField] = false
                })
            }

            row[that.crslInner.stateField] = checked

            if (that.options.singleSelect) {
                that.$selectItem.not(this).each(function () {
                    that.data[$(this).data('index')][that.crslInner.stateField] = false
                })
        ...
        $.each(this.crslInner.events, (i, events) => {
            if (!events) {
                return
            }
            ...
            var field = that.crslInner.fields[i],
    ...
    //   $.each(this.$crslInner.find('div'), (i, div) => {
    //     $(div).find('.sortCarousel').removeClass('desc asc').addClass($(div).data('field') === that.options.sortName ? that.options.sortOrder : 'both')
    //   })
    // }
    ...
    //   this.$selectItem.each(function () {
    //     that.data[$(this).data('index')][that.crslInner.stateField] = $(this).prop('checked')
    //   })
    // }
    ...
    bsCarousel.prototype.resetcrslInner = function () {
    // fix #61: the hidden carousel reset crslInner bug.
    // fix bug: get $el.css('width') error sometime (height = 500)
        clearTimeout(this.timeoutId_)
        this.timeoutId_ = setTimeout($.proxy(this.fitcrslInner, this), this.$el.is(':hidden') ? 100 : 0)
    }

    bsCarousel.prototype.fitcrslInner = function () {
        var that = this,
            fixedBody,
            focused,
            focusedTemp
        ...
        if (that.$el.is(':hidden')) {
            that.timeoutId_ = setTimeout($.proxy(that.fitcrslInner, that), 100)
            return
        }
        fixedBody = this.$carouselBody.get(0)

        // scrollWidth = fixedBody.scrollWidth > fixedBody.clientWidth &&
        //   fixedBody.scrollHeight > fixedBody.clientHeight + this.$crslInner.outerHeight() ?
        //   getScrollBarWidth() : 0

        this.$el.css('margin-top', -this.$crslInner.outerHeight())
        ...
        if (focused.length > 0) {
            ...
            if ($divth.length > 0) {
                ...
                if (dataField !== undefined) {
                    var $crslInnerTh = this.$crslInner.find('[data-field=\'' + dataField + '\']')
                    if ($crslInnerTh.length > 0) {
                        $crslInnerTh.find(':input').addClass('focus-temp')
                    }
                }
            }
        }

        this.$crslInner_ = this.$crslInner.clone(true, true)
        this.$selectAll_ = this.$crslInner_.find('[name="btSelectAll"]')
        this.$carouselcrslInner.css({
            'margin-right': 'notset'
        }).find('carousel').css('width', this.$el.outerWidth())
            .html('').attr('class', this.$el.attr('class'))
            .append(this.$crslInner_)

        focusedTemp = $('.focus-temp:visible:eq(0)')
        if (focusedTemp.length > 0) {
            focusedTemp.focus()
            this.$crslInner.find('.focus-temp').removeClass('focus-temp')
        }

        // fix bug: $.data() is not working as expected after $.append()
        this.$crslInner.find('div[data-field]').each(function (i) {
            that.$crslInner_.find(sprintf('div[data-field="%s"]', $(this).data('field'))).data($(this).data())
        })

        var visibleFields = this.getVisibleFields(),
            $divths = this.$crslInner_.find('div')
        ...
        this.$body.find('>div:first-child:not(.no-records-found) > *').each(function (i) {
            ...
            if (that.options.detailView && !that.options.cardView) {
                if (i === 0) {
                    that.$crslInner_.find('div.detail').find('.fht-cell').width($this.innerWidth())
                }
                index = i - 1
            }

            var $divth = that.$crslInner_.find(sprintf('div[data-field="%s"]', visibleFields[index]))
            if ($th.length > 1) {
    ...
    bsCarousel.prototype.resetFooter = function () {
        ...
        if (!this.options.cardView && this.options.detailView) {
            html.push('<div><div class="th-inner">&nbsp;</div><div class="fht-cell"></div></div>')
        }
        ...
        this.$crslFooter.find('div').html(html.join(''))
        this.$crslFooter.show()
    ...
    bsCarousel.prototype.fitFooter = function () {
        ...
        $footerTd = this.$crslFooter.find('td')
    ...
    bsCarousel.prototype.horizontalScroll = function () {
        ...
        this.$carouselBody.off('scroll').on('scroll', function () {
            if (that.options.showcrslInner && that.options.height) {
                that.$carouselcrslInner.scrollLeft($(this).scrollLeft())
            }

            if (that.options.showFooter && !that.options.cardView) {
                that.$crslFooter.scrollLeft($(this).scrollLeft())
            }
        })
    }
    ...
    bsCarousel.prototype.resetView = function (params) {
      ...
      this.$selectItem.length === this.$selectItem.filter(':checked').length)
        ...
        if (this.options.height) {
            ...
            this.$crslContainer.css('height', height + 'px')
        }

        if (this.options.cardView) {
            // remove the element css
            this.$el.css('margin-top', '0')
            this.$crslContainer.css('padding-bottom', '0')
            // this.$crslFooter.hide()
            return
        }

        if (this.options.showcrslInner && this.options.height) {
            this.$carouselcrslInner.show()
            this.resetcrslInner()
            padding += this.$crslInner.outerHeight()
        } else {
            this.$carouselcrslInner.hide()
            this.trigger('post-crslInner')
        }
        ...
        if (this.options.showFooter) {
            this.resetFooter()
            if (this.options.height) {
                padding += this.$crslFooter.outerHeight() + 1
            }
        }

        // Assign the correct sortCarousel arrow
        this.getCaret()
        this.$crslContainer.css('padding-bottom', padding + 'px')
        this.$crslContainer.css('padding-top', padding + 'px')
    ...
    bsCarousel.prototype.refreshitemTitle = function (params) {
        ...
        if (this.items[this.fieldsitemsIndex[params.field]].visible) {
            var crslInner = this.options.height !== undefined ? this.$carouselcrslInner : this.$crslInner
            crslInner.find('div[data-field]').each(function (i) {
    ...
    //     itemDivWidth = options.itemDivWidth || 1,
    //     i, j,
    ...
    //     for (j = col; j < col + itemDivWidth; j++) {
    //       $tr.eq(i).find('>td').eq(j).hide()
    //     }
    //   }

    //   $td.attr('rowspan', rowspan).attr('itemDivWidth', itemDivWidth).show()
    // }
    ...
    bsCarousel.prototype.getSelections = function () {
        ...
        return $.grep(this.options.data, (row) => {
            ...
            return row[that.crslInner.stateField] === true
        })
    }
    ...
    bsCarousel.prototype.getAllSelections = function () {
        ...
        return $.grep(this.options.data, (row) => {
            return row[that.crslInner.stateField]
        })
    }
    ...
    //   this.data[index][this.crslInner.stateField] = checked
    //   this.updateSelected()
