...
class Toggler {
    constructor(options) {
        ...
        var openPanelIndex = options.openPanelIndex, oneOpenPanel = options.oneOpenPanel || false, panelControllerClass = 'js-panelController', titleClass = 'js-panelController__title', contentClass = 'js-targetPanel', closedClass = 'is-closed', openedClass = 'is-open';
        try {
         ...
         */
        function render() {
            if (typeof element === undefined || typeof element === 'undefined' || element === null)
                return;
            // attach classes to buttons and containers
            [].forEach.call(element.querySelectorAll(`.${panelControllerClass}`), function (item, index) {
        ...
        function onClick(e) {
            ...
            else if (oneOpenPanel && evTarget.classList.contains(closedClass)) {
                closeAll();
                toggle(e.target.nextElementSibling);
                return;
            }
            else if (!oneOpenPanel) {
                toggle(e.target.nextElementSibling);
            }
        }
        /**
